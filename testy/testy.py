...
from matchers.matcher import eq_to_Nieistniejace_Instance
import hamcrest
...
class Test_przedmiot_class_INIT(unittest.TestCase):
    ...
    def test_check_if_przedmiot_unique_success_przedmiot_list(self):
        ...
        assert_that(self.instance_2.przedmiot_list).is_equal_to(
            ['Matematyka', 'Polski', 'Astronomia', 'Szachy', 'Chemia'])
...
@params(('prz1', '', 'Panda 3', EmptyNameField), ('prz2', 'Angielski', '', EmptyTeacherField),
        ('prz3', '', '', ValueError), ("NieistniejacyPrzedmiot", "NieistniejacyPrzedmiot", "Resnick", ValueError))
...
class Test_uczen_class_GET_WSZYSCY_UCZNIOWIE(unittest.TestCase):
    def test_get_wszyscy(self):
        ...
        assert_that(Uczen.get_wszyscy_uczniowie()).is_equal_to(
            ['id:0, imie:Test, nazwisko:User, oceny:3, uwagi:Brak uwag'])
...
class Test_uczen_class_GET_UWAGI(unittest.TestCase):
    @patch('src.uczen.Uczen.get_instance')
...
class Test_uczen_class_GET_OCENY(unittest.TestCase):
    ...
    def test_get_oceny_failure(self, get_instance_mock):
        get_instance_mock.return_value = "Nieistniejace instance"
        hamcrest.assert_that(Uczen.get_oceny(0), hamcrest.is_(eq_to_Nieistniejace_Instance()))


class Test_uczen_class_TEST_INPUT(unittest.TestCase):
    def setUp(self):
        self.mock_input = Mock(side_effect=[('', 'przyroda', ''),(1, 'przyroda', 3),(1, 'Angielski', 8)])

    def test_uczen_test_fail_empty_fields(self):
        a,b,c=self.mock_input()
        assert_that(Uczen.testInput).raises(ValueError).when_called_with(a,b,c)
...
class Test_uczen_class_DODAJ_OCENE(unittest.TestCase):
    @patch('src.uczen.Uczen.testInput')
    @patch('src.uczen.Uczen.get_instance')
    @patch('src.przedmiot.Przedmiot.get_nauczyciel')
...
class Test_uczen_class_DODAJ_Uwaga(unittest.TestCase):
    @patch('src.uczen.Uczen.get_instance')
    @patch('src.przedmiot.Przedmiot.get_nauczyciel')
    def test_dodaj_uwage_success(self, mock_get_nauczyciel, mock_getInstanceOfStudent):
        correct_input_check = mock.create_autospec(Uczen.testInputUwaga)
        correct_input_check.return_value = 1
        Uczen.id = 0
        Uczen.id_uwagi = 0
        mock_nazwa_przedmiotu_getter = mock.MagicMock()
        type(mock_nazwa_przedmiotu_getter).content = mock.PropertyMock(return_value="Angielski")
        mock_getInstanceOfStudent.return_value.uwagi = []
        Przedmiot.przedmiot_list = [mock_nazwa_przedmiotu_getter.content]
        mock_get_nauczyciel.return_value = "Jan Kowalski"
    ...
    def test_dodaj_uwage_failure(self):
        incorrect_input_check = mock.create_autospec(Uczen.testInputUwaga)
        incorrect_input_check.return_value = -1
        assert_that(Uczen.dodaj_uwage).raises(ValueError).when_called_with(0, "1", "2")
...
class test_uczen_show_oceny_from_teacher(unittest.TestCase):
    ...
    def test_uczen_show_oceny_from_teacher_success(self):
        assert_that(Uczen.show_oceny_from_teacher("Joanna Czarnowska")).is_equal_to(
            "Imię ucznia: Lorem, nazwisko ucznia: Ipsum, ocena: 3, Imię ucznia: Test, nazwisko ucznia: User, ocena: 2, ")
